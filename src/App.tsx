...
// UAE Labour Law Decree-Law No. 33 of 2021 gratuity formula
...
// Future value of monthly EOS contributions if invested at annualRate
                    ...
                    <BarChart data={mockChartData} margin={{ top: 0, right: 10, left: 10, bottom: 0 }}>
                      <Tooltip 
                        ...
                        contentStyle={{ 
                          ...
                          border: '1px solid rgba(162, 179, 165, 0.2)',
                          borderRadius: '8px',
                          color: '#F9F9F9',
                          fontSize: '12px'
                        }}
            ...
            >
              <motion.div 
                animate={{ 
                  scale: [1, 1.05, 1],
                  boxShadow: [
                    "0px 0px 0px 0px rgba(86, 168, 97, 0)",
                    "0px 0px 12px 2px rgba(86, 168, 97, 0.3)",
                    "0px 0px 0px 0px rgba(86, 168, 97, 0)"
                  ]
                }}
...
const TrustBar = () => {
  const uaeCompanies = [
    { name: "Emirates", icon: <Plane className="w-6 h-6" />, style: "font-serif text-xl tracking-wide" },
    { name: "EMAAR", icon: <Building className="w-6 h-6" />, style: "font-sans font-bold text-xl tracking-widest" },
    { name: "etisalat", icon: <Wifi className="w-6 h-6" />, style: "font-sans font-medium text-xl lowercase tracking-tight" },
    { name: "DP WORLD", icon: <Ship className="w-6 h-6" />, style: "font-sans font-black text-xl tracking-tighter" },
    { name: "ADNOC", icon: <Droplet className="w-6 h-6" />, style: "font-sans font-bold text-xl tracking-wider" },
  ];
  ...
  <section className="py-16 md:py-24 bg-vestora-white dark:bg-[#0B120E] transition-colors duration-300">
    <div className="max-w-3xl mx-auto px-6 text-center">
      <FadeIn>
        ...
        <h2 className="text-3xl md:text-4xl font-bold text-vestora-charcoal dark:text-vestora-neutral tracking-tight mb-6">
          UAE companies face a new EOS mandate. Most have no way to compare their options.
        </h2>
        <p className="text-lg text-vestora-charcoal/70 dark:text-vestora-neutral/70 leading-relaxed">
          Cabinet Resolution 96/2023 requires companies to invest employee end-of-service benefits in SCA-regulated funds. But there is no single place to compare funds, calculate liabilities, or connect with fund managers. Until now.
        </p>
      </FadeIn>
    </div>
  </section>
...
const ValueProposition = () => {
  ...
  return (
    <section className="py-16 md:py-32 bg-vestora-white dark:bg-[#0B120E] transition-colors duration-300 relative overflow-hidden" id="product">
      ...
      <RippleBackground />
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-12 mb-16 md:mb-32">
          <FadeIn className="flex-1">
            <h2 className="text-3xl md:text-5xl lg:text-6xl font-bold text-vestora-charcoal dark:text-vestora-neutral tracking-tight">
              Everything You Need to Manage EOS Investment
            </h2>
          </FadeIn>
            ...
            <FadeIn key={idx} delay={idx * 0.1}>
              ...
              >
                <motion.div 
                  ...
                  className="w-12 h-12 rounded-xl bg-vestora-forest/10 dark:bg-vestora-growth/10 text-vestora-forest dark:text-vestora-growth flex items-center justify-center mb-6"
                >
                  {pillar.icon}
                </motion.div>
                <h3 className="text-xl font-bold text-vestora-charcoal dark:text-vestora-neutral mb-3">{pillar.title}</h3>
                <p className="text-vestora-charcoal/70 dark:text-vestora-neutral/70 leading-relaxed">{pillar.desc}</p>
              </motion.div>
            </FadeIn>
          ))}
        </div>
        ...
        {showModal && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => { setShowModal(false); setSubmitted(false); setForm({ name: '', company: '', email: '' }); setErrors({ name: '', email: '' }); }}
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
              ...
              >
                <button
                  onClick={() => { setShowModal(false); setSubmitted(false); setForm({ name: '', company: '', email: '' }); setErrors({ name: '', email: '' }); }}
                  className="absolute top-4 right-4 p-1.5 rounded-full text-vestora-sage hover:text-vestora-charcoal dark:hover:text-vestora-neutral hover:bg-vestora-sage/10 transition-colors"
                >
                  <X className="w-4 h-4" />
                </button>
                ...
                {!submitted ? (
                  ...
                  <div className="p-8 text-center">
                    ...
                    <a
                      href="#"
                      className="text-sm font-semibold text-vestora-forest dark:text-vestora-growth hover:underline inline-flex items-center gap-1"
                    >
